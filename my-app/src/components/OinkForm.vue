...
import { sendOink } from '@/api'
import { useWallet } from 'solana-wallets-vue'

// Props.
const props = defineProps({
    forcedArgument: String,
})
const { forcedArgument } = toRefs(props)

// Form data.
const content = ref('')
const argument = ref('')
const slugArgument = useSlug(argument)
const effectiveArgument = computed(() => forcedArgument.value ?? slugArgument.value)
...
const canOink = computed(() => content.value && characterLimit.value > 0)

// Actions.
const emit = defineEmits(['added'])
const send = async () => {
    if (! canOink.value) return
    const oink = await sendOink(effectiveArgument.value, content.value)
    emit('added', oink)
    argument.value = ''
    content.value = ''
}
...
<template>
        ...
        <div class="flex flex-wrap items-center justify-between -m-2">
            ...
            <div class="relative m-2 mr-4">
                <input
                    ...
                    :value="effectiveArgument"
                    :disabled="forcedArgument"
                    @input="argument = $event.target.value"
                >
                <div class="absolute left-0 inset-y-0 flex pl-3 pr-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 m-auto" :class="effectiveArgument ? 'text-pink-500' : 'text-gray-400'" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="flex items-center space-x-6 m-2 ml-auto">
                ...
                <!-- Oink button. -->
                <button
                    class="text-white px-4 py-2 rounded-full font-semibold" :disabled="! canOink"
                    :class="canOink ? 'bg-pink-500' : 'bg-pink-300 cursor-not-allowed'"
                    @click="send"
                >
                    Oink
                </button>
    ...
    <div v-else class="px-8 py-4 bg-gray-50 text-gray-500 text-center border-b">
        Connect your wallet to start oinking...
    </div>
</template>
